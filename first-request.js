
var request = require('request');

// https://query.yahooapis.com/v1/public/yql?q=select%20astronomy.sunset%20from%20weather.forecast%20where%20woeid%20in%20(select%20woeid%20from%20geo.places(1)%20where%20text%3D%22maui%2C%20hi%22)&format=json&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys

request('https://weather-ydn-yql.media.yahoo.com/forecastrss?location=sunnyvale,ca&format=json', function (error, response, body) {
  if(!error && response.statusCode == 200){
    // this will display the value AS A STRING AND NOT AS JAVASCRIPT OBJECT
    // console.log(body);

    // since body is type STRING, it needs to be converted as JSON and assign it to a variable 
    // var parsedData = JSON.parse(body);
    // console.log("Sunset in Hawaii is at...")
    // console.log(parsedData["query"]["results"]["channel"]["astronomy"]["sunset"]);
    console.log(body);
  } else {
    console.log(`Something went wrong: ${error}`);
  }
});

// THIS YAHOO API IS NOT SUPPORTED ANYMORE